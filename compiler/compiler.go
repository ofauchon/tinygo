...
func (b *builder) createFunctionDefinition() {
	...
	if !b.fn.LLVMFn.IsDeclaration() {
		b.addError(b.fn.Pos(), "function is already defined:"+b.fn.LLVMFn.Name())
		return
	}
	if !b.fn.IsExported() {
	...
	if b.fn.IsInterrupt() && strings.HasPrefix(b.Triple(), "avr") {
		b.fn.LLVMFn.SetFunctionCallConv(85) // CallingConv::AVR_SIGNAL
	}
